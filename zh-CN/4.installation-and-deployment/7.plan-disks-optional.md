...
* 数据盘
  ...
  数据盘的配置参数为 `data_dir`。数据盘用来存储基线数据。在您首次启动 OceanBase 数据库时，`${data_dir}/sstable` 将自动创建。数据盘的大小由` datafile_size`  或 `datafile_disk_percentage` 参数决定，其中` datafile_size` 优先级高于 `datafile_disk_percentage`, `datafile_disk_percentage` 的默认值为 `90`。目前，OceanBase 数据库只允许磁盘空间扩容, 不支持磁盘空间缩容。
  磁盘扩容操作:
  在系统租户下, 执行命令:
  ```
